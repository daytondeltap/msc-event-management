...
(() => {
  ...
  let sourceId='', nodeDrag=null, toolbar=null, raf=0;
  ...
  document.addEventListener('pointermove',e=>{if(!nodeDrag)return;if(Math.hypot(e.clientX-nodeDrag.x,e.clientY-nodeDrag.y)>3)nodeDrag.moved=true;if(!nodeDrag.moved||raf)return;raf=requestAnimationFrame(()=>{raf=0;const p=worldPoint(e.clientX,e.clientY),c=state.connections.find(v=>v.id===nodeDrag.id);if(!p||!c)return;const others=layout(visible()).filter(v=>v.c.id!==c.id).map(v=>v.n),rects=visible().map(rect),n=safeNode(p,others,rects);c.nodeMode='manual';c.node=n;refresh()})},{capture:true,passive:true});
  document.addEventListener('pointerup',e=>{if(!nodeDrag)return;const d=nodeDrag;nodeDrag=null;if(d.moved){save(true);refresh()}else openToolbar(d.id,e.clientX,e.clientY)},true);
