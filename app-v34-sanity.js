...
(() => {
  ...
  // v24 creates its editor on pointerup. A legacy class in that editor is hidden by
  // features-v20.css, so replace only that hidden UI with the supported v20 editor shell.
  ...
  // Register before v33. Pointer activation is handled here and the following synthetic
  // click is swallowed, while keyboard-generated clicks still toggle normally.
  document.addEventListener('pointerup', event => {
    if(!targetClosest(event,'#mobileMoreButton')) return;
    event.preventDefault();
    event.stopImmediatePropagation();
    morePointerUntil = performance.now() + 500;
    toggleMore();
  }, true);
  ...
  document.addEventListener('click', event => {
    if(targetClosest(event,'#mobileMoreButton')){
      event.preventDefault();
      event.stopImmediatePropagation();
      if(performance.now() < morePointerUntil) return;
      toggleMore();
      return;
    }
