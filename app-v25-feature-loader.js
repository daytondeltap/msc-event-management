...
(() => {
  ...
  function load(src,timeout=6000){
    ...
    return new Promise((resolve,reject)=>{
      ...
      function finish(ok,v){
        ...
        if(ok){loaded.add(src);resolve();}
        else{s.remove();reject(v);}
      }
      s.onload=()=>finish(true);s.onerror=()=>finish(false,new Error(`load:${src}`));s.src=src;s.async=false;document.body.appendChild(s);
    });
  }
  ...
  async function loadWorkspace(){
    ...
    workspacePromise=(async()=>{
      try{
        ...
        const boardSetView=setView;window.MSC_V10=window.MSC_V10||{};window.MSC_V10.renderBoards=()=>boardSetView('boards',false);reassert();
        ...
        console.error('MSC Boards/Share failed',err);toast('Boards/Share could not finish loading — tap again to retry');return false;
      }finally{
  ...
  async function loadMaps(){
    ...
    mapsPromise=(async()=>{
      try{
        if(!document.querySelector('link[data-msc-leaflet]')){const l=document.createElement('link');l.rel='stylesheet';l.href='https://unpkg.com/leaflet@1.9.4/dist/leaflet.css';l.dataset.mscLeaflet='1';document.head.appendChild(l)}
        await load('https://unpkg.com/leaflet@1.9.4/dist/leaflet.js',8000);
        await load('app-osm.js');reassert();
        await load('app-v8-osm-search.js');reassert();
        F.mapsReady=true;if(view==='venues')try{venues()}catch{}
        return true;
      }catch(err){
        console.warn('OpenStreetMap unavailable',err);toast('Map service unavailable — tap Venues again to retry');return false;
      }finally{
  ...
  document.addEventListener('click',async e=>{const boards=e.target.closest?.('[data-view="boards"]');if(boards&&!F.workspaceReady){e.preventDefault();e.stopImmediatePropagation();const ok=await loadWorkspace();if(ok){setView('boards');patchBoardButtons()}return}if(e.target.closest?.('#shareButton')&&!F.workspaceReady){e.preventDefault();e.stopImmediatePropagation();const ok=await loadWorkspace();if(ok)document.getElementById('shareButton')?.click();return}if(e.target.closest?.('[data-view="venues"]')&&!F.mapsReady)setTimeout(loadMaps,0)},true);
